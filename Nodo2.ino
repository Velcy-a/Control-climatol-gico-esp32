...
// Callback normal para verificar el estatus del envío de datos (Core 3.x)
void OnDataSent(const wifi_tx_info_t *txInfo, esp_now_send_status_t status) {
  Serial.print("Envío Nodo 2: ");
...
void setup() {
  ...
  // 5. Inicialización estándar del sensor SCD4X
  scd4x.begin(Wire);
...
void loop() {
  ...
  if (dataReady) {
    ...
    datos.co2 = co2_val; // Captura exclusiva de CO2
    datos.temp1 = s_t;
    datos.hum1 = s_h;
  }

  datos.pres = 0.0; 
  datos.tvoc = 0; // Nodo 2 no reporta COV
  datos.temp2 = 0.0; 
  datos.hum2 = 0.0;
